  ...
  <div>
    ...
    >
      ...
      <template #context-menu>
        ...
        <v-list-item
          v-show="isFile"
          v-for="(menuItem, index) in contextMenuItems.jobFileList"
          :key="index"
...
export default {
  computed: {
    ...
    volume: {
      ...
      set(value) {
        this.directory =
          value === Path.getVolume(this.gCodesDirectory)
            ? this.gCodesDirectory
            : `${value}:`;
      },
    },
  },
  ...
  methods: {
          ...
          if (!file.isDirectory) {
            ...
            try {
              ...
              if (Path.isGCodePath(file.name, this.gCodesDirectory)) {
                ...
                file.simulatedTime = fileInfo.simulatedTime
                  ? fileInfo.simulatedTime
                  : null;
              }
            } catch (e) {
    ...
    fileClicked(item) {
      if (!this.isPrinting) {
        this.startJobDialog.title = this.$t("dialog.startJob.title", [
          item.name,
        ]);
        this.startJobDialog.prompt = this.$t("dialog.startJob.prompt", [
          item.name,
        ]);
  ...
  watch: {
    gCodesDirectory(to, from) {
      if (
        Path.equals(this.directory, from) ||
        !Path.startsWith(this.directory, to)
      ) {
        this.directory = to;
      }
    },
