...
<template>
  <v-card>
    <v-card-text
      ...
      v-show="sensorsPresent || visibleAxes.length + move.extruders.length"
    >
      <!-- Axis Positions -->
          ...
          <v-col tag="strong" class="category-header">
            ...
            >
              {{
                $t(
                  displayToolPosition
                    ? "panel.status.toolPosition"
                    : "panel.status.machinePosition"
                )
              }}
            </a>
          </v-col>
              ...
              >
                <strong>
                  ...
                  <v-icon v-if="isFilamentSensorPresent(index)" small>
                    {{
                      sensors.filamentSensors[index].filamentPresent
                        ? "mdi-check"
                        : "mdi-window-close"
                    }}
                  </v-icon>
      ...
      <template
        v-show="
          isNumber(move.currentMove.requestedSpeed) ||
          isNumber(move.currentMove.topSpeed)
        "
      >
        <v-divider
          v-show="move.axes.length + move.extruders.length"
          class="my-2"
        ></v-divider>
        ...
        <v-row align-content="center" no-gutters class="flex-nowrap">
          ...
          <v-col>
            <v-row align-content="center" no-gutters>
              <v-col
                v-if="isNumber(move.currentMove.requestedSpeed)"
                class="d-flex flex-column align-center"
              >
                <strong>
              ...
              <v-col
                v-if="isNumber(move.currentMove.topSpeed)"
                class="d-flex flex-column align-center"
              >
                <strong>
        ...
        <v-row align-content="center" no-gutters class="flex-nowrap">
          ...
          <v-col>
            <v-row align-content="center" justify="center" no-gutters>
              <v-col
                v-if="boards.length && boards[0].vIn.current > 0"
                class="d-flex flex-column align-center"
              >
                <strong>
                  {{ $t("panel.status.vIn") }}
                ...
                <v-tooltip bottom>
                  ...
                  {{
                    $t("panel.status.minMax", [
                      $display(boards[0].vIn.min, 1, "V"),
                      $display(boards[0].vIn.max, 1, "V"),
                    ])
                  }}
                </v-tooltip>
              </v-col>
              ...
              <v-col
                v-if="boards.length && boards[0].v12.current > 0"
                class="d-flex flex-column align-center"
              >
                <strong>
                  {{ $t("panel.status.v12") }}
                ...
                <v-tooltip bottom>
                  ...
                  {{
                    $t("panel.status.minMax", [
                      $display(boards[0].v12.min, 1, "V"),
                      $display(boards[0].v12.max, 1, "V"),
                    ])
                  }}
                </v-tooltip>
              </v-col>
              ...
              <v-col
                v-if="boards.length && boards[0].mcuTemp.current > -273"
                class="d-flex flex-column align-center"
              >
                ...
                <v-tooltip bottom>
                  ...
                  {{
                    $t("panel.status.minMax", [
                      $display(boards[0].mcuTemp.min, 1, "C"),
                      $display(boards[0].mcuTemp.max, 1, "C"),
                    ])
                  }}
                </v-tooltip>
              </v-col>
              ...
              >
                ...
                <div class="d-flex-inline">
                  <span
                    v-for="(probe, index) in sensors.probes"
                    :key="index"
...
export default {
  computed: {
    ...
    probesPresent() {
      return this.sensors.probes.some(
        (probe) => probe && probe.type !== ProbeType.none
      );
    },
    sensorsPresent() {
      return (
  ...
  methods: {
    ...
    isFilamentSensorPresent(extruderIndex) {
      return (
        extruderIndex < this.sensors.filamentMonitors.length &&
        this.sensors.filamentMonitors[extruderIndex] &&
        this.sensors.filamentMonitors[extruderIndex].enabled &&
        this.sensors.filamentMonitors[extruderIndex].filamentPresent instanceof
          Boolean
      );
    },
