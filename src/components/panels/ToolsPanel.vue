...
<template>
  <v-card>
    <v-card-title class="py-2">
      ...
      <panel-link
        :active="currentPage !== 'extra'"
        @click="currentPage = 'extra'"
      >
        <v-icon small>mdi-plus</v-icon> {{ $t("panel.tools.extra.caption") }}
      </panel-link>

      <panel-link>
        <span class="item" @click="toggleOffset">Offsets</span>
      </panel-link>
      ...
      >
        ...
        <v-card>
          <v-layout justify-center column class="pt-2 px-2">
            ...
            <tool-input
              ref="allActive"
              :label="$t('panel.tools.allActiveTemperatures')"
              all
              active
            ></tool-input>
            <tool-input
              :label="$t('panel.tools.allStandbyTemperatures')"
              all
              standby
            ></tool-input>
          </v-layout>
    ...
    <v-card-text class="pa-0">
      <template v-if="currentPage === 'tools'">
          ...
          <tbody>
            ...
            <template v-for="(tool, toolIndex) in visibleTools">
              ...
              <tr
                ...
                :key="`tool-${toolIndex}-${toolHeaterIndex}`"
                :class="{
                  [selectedToolClass]: tool.number === state.currentTool,
                }"
              >
                <!-- Tool Name -->
                <th
                  ...
                  <span class="font-weight-regular caption">
                    ...
                    <template v-if="isConnected && canLoadFilament(tool)">
                      ...
                      <a
                        ...
                        href="javascript:void(0)"
                        @click="
                          filamentMenu.tool = tool;
                          filamentMenu.dialogShown = true;
                        "
                      >
                ...
                <template v-if="!toolHeater && getSpindle(tool)">
                  ...
                  <td>
                    <tool-input
                      :spindle="getSpindle(tool)"
                      :spindle-index="getSpindleIndex(tool)"
                      active
                    ></tool-input>
                  </td>
              ...
              <!-- Divider -->
              <tr
                v-if="toolIndex < visibleTools.length - 1"
              ...
              <template v-if="bedHeater">
                ...
                <tr :key="`bed-${bedIndex}-0`">
                  ...
                  <th class="pl-2">
                    <a
                      href="javascript:void(0)"
                      @click="bedHeaterClick(bedHeater, bedIndex)"
                    >
                  ...
                  <th>
                    <a
                      href="javascript:void(0)"
                      @click="bedHeaterClick(bedHeater, bedIndex)"
                      :class="getHeaterColor(heat.bedHeaters[bedIndex])"
                    >
              ...
              <template v-if="chamberHeater">
                ...
                <tr :key="`chamber-${chamberIndex}-0`">
                  ...
                  <th class="pl-2">
                    ...
                    >
                      {{
                        $t("panel.tools.chamber", [
                          hasOneChamber ? "" : chamberIndex,
                        ])
                      }}
                    </a>
                  </th>
                  ...
                  <th>
                    <a
                      href="javascript:void(0)"
                      @click="chamberHeaterClick(chamberHeater, chamberIndex)"
                      :class="getHeaterColor(heat.chamberHeaters[chamberIndex])"
                    >
                      {{
                        getHeaterName(
                          chamberHeater,
                          heat.chamberHeaters[chamberIndex]
                        )
                      }}
                    </a>
                  ...
                  <td class="pl-2 pr-1">
                    <tool-input
                      :chamber="chamberHeater"
                      :chamber-index="chamberIndex"
                      active
                    ></tool-input>
                  </td>
        ...
        <reset-heater-fault-dialog
          :shown.sync="resetHeaterFault"
          :heater="faultyHeater"
        ></reset-heater-fault-dialog>
        <filament-dialog
          :shown.sync="filamentMenu.dialogShown"
          :tool="filamentMenu.tool"
        ></filament-dialog>
      </template>
      ...
      <template v-else-if="currentPage === 'extra'">
        <table class="extra ml-2 mr-2" v-show="extraSensors.length">
          ...
          <tbody>
            <tr
              v-for="extraItem in extraSensors"
              :key="`extra-${extraItem.index}`"
            >
              <td class="hidden-sm-and-down">
                <v-switch
                  class="ml-3"
                  :input-value="
                    displayedExtraTemperatures.indexOf(extraItem.index) !== -1
                  "
                  @change="toggleExtraVisibility(extraItem.index)"
...
import {
  AnalogSensorType,
  HeaterState,
} from "../../store/machine/modelEnums.js";
...
export default {
  computed: {
        ...
        (this.tools.some(
          (tool) =>
            ...
            tool.heaters.some(
              (toolHeater) =>
                toolHeater >= 0 &&
                toolHeater < this.heat.heaters.length &&
                this.heat.heaters[toolHeater] &&
                this.heat.heaters[toolHeater].state !== HeaterState.off,
              this
            ),
          this
        ) ||
          this.heat.bedHeaters.some(
            (bedHeater) =>
              bedHeater >= 0 &&
              bedHeater < this.heat.heaters.length &&
              this.heat.heaters[bedHeater] &&
              this.heat.heaters[bedHeater].state !== HeaterState.off,
            this
          ) ||
          this.heat.chamberHeaters.some(
            (chamberHeater) =>
              chamberHeater >= 0 &&
              chamberHeater < this.heat.heaters.length &&
              this.heat.heaters[chamberHeater] &&
              this.heat.heaters[chamberHeater].state !== HeaterState.off,
            this
          ))
      );
    },
    ...
    extraSensors() {
      return this.sensors.analog
        ...
        .filter(
          (item) =>
            item.sensor &&
            !this.heat.heaters.some(
              (heater) => heater && heater.sensor === item.index
            )
        );
    },
    bedHeaters() {
      return this.heat.bedHeaters.map((heaterIndex) => {
        if (
          heaterIndex >= 0 &&
          heaterIndex < this.heat.heaters.length &&
          this.heat.heaters[heaterIndex]
        ) {
    ...
    chamberHeaters() {
      return this.heat.chamberHeaters.map((heaterIndex) => {
        if (
          heaterIndex >= 0 &&
          heaterIndex < this.heat.heaters.length &&
          this.heat.heaters[heaterIndex]
  ...
  methods: {
    ...
    toggleOffset() {
      this.$parent.showOffsetModel = !this.$parent.showOffsetModel;
    },
    ...
    async turnEverythingOff() {
      ...
      try {
        ...
        if (!(e instanceof DisconnectedError)) {
          this.$log(
            "error",
            this.$t("error.turnOffEverythingFailed"),
            e.message
          );
        }
      }
    ...
    getHeaterName(heater, heaterIndex) {
      if (
        heater &&
        heater.sensor >= 0 &&
        heater.sensor < this.sensors.analog.length
      ) {
    ...
    getFilament(tool) {
      if (
        this.isNumber(tool.filamentExtruder) &&
        tool.filamentExtruder >= 0 &&
        tool.filamentExtruder < this.move.extruders.length
      ) {
