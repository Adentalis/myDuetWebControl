  ...
  >
    ...
    <v-col>
      <v-combobox
        ...
        @keyup.enter="send"
        @change="change"
        @blur="wasFocused = showItems = false"
        @click="click"
...
export default {
      ...
      if (this.showItems && !this.disableAutoComplete) {
        ...
        return this.codes
          .filter(
            (code) =>
              currentCode === "" ||
              code.toLowerCase().indexOf(currentCode) !== -1
          )
  ...
  methods: {
    ...
    hasUnprecedentedParameters: (code) =>
      !code || /(M23|M28|M30|M32|M36|M117)[^0-9]/i.test(code),
    async send() {
      this.showItems = false;

      const code =
        !this.code || this.code.constructor === String
          ? this.code
          : this.code.value;
      if (code && code.trim() !== "" && !this.doingCode) {
        ...
        if (
          ...
          !conditionalKeywords.some((keyword) =>
        ...
        this.doingCode = true;
        try {
          const reply = await this.sendCode({
            code: codeToSend,
            fromInput: true,
          });
          if (
            ...
            this.codes.indexOf(codeToSend.trim()) === -1
          ) {
