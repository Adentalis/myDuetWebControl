...
const store = new Vuex.Store({
  state: {
    control: {
      border: "rgb(68, 68, 241,1)",
      background: "rgb(100, 149, 237,1)",
    },
    setting: {
      border: "rgb(76, 23, 119,1)",
      background: "rgb(167, 88, 219,1)",
    },
    isConnecting: false,
  ...
  getters: {
    ...
    connectedMachines: () =>
      Object.keys(machines).filter((machine) => machine !== defaultMachine),
    isConnected: (state) =>
      state.selectedMachine !== defaultMachine &&
      state.machine &&
      !state.machine.isReconnecting,
    uiFrozen: (state, getters) =>
      state.isConnecting || state.isDisconnecting || !getters.isConnected,
  },
  actions: {
    updateColourPanels({ commit }, payload) {
      commit("updateSettingsPanelsColour", payload.setting);
      commit("updateControlPanelsColour", payload.control);
    },
    ...
    async connect(
      ...
      try {
        const connectorInstance = await connector.connect(
          hostname,
          username,
          password
        );
        const module = machine(
          connectorInstance,
          pluginCacheFields,
          pluginSettingFields
        );
    ...
    async disconnect(
      { state, commit, dispatch },
      { hostname = state.selectedMachine, doDisconnect = true } = {}
    ) {
      if (!hostname || hostname === defaultMachine) {
        throw new Error("Invalid hostname");
      }
      ...
      if (doDisconnect) {
        ...
        try {
          ...
          logGlobal(
            "warning",
            i18n.t("error.disconnect", [hostname]),
            e.message
          );
          console.warn(e);
        }
    ...
    async onConnectionError({ state, dispatch, commit }, { hostname, error }) {
      if (error instanceof InvalidPasswordError) {
        logGlobal(
          "error",
          i18n.t("events.connectionLost", [hostname]),
          error.message
        );
        ...
        logGlobal(
          "warning",
          i18n.t("events.reconnecting", [hostname]),
          error.message
        );
    ...
    async loadDwcPlugin({ state, dispatch, commit }, { name, saveSettings }) {
      ...
      if (plugin.dwcVersion && !checkVersion(plugin.dwcVersion, version)) {
        throw new Error(
          `Plugin ${name} requires incompatible DWC version (need ${plugin.dwcVersion}, got ${version})`
        );
      }
      ...
      for (let i = 0; i < plugin.dwcDependencies.length; i++) {
        ...
        if (!dependentPlugin) {
          throw new Error(
            `Failed to find DWC plugin dependency ${plugin.dwcDependencies[i]} for plugin ${plugin.name}`
          );
        }
  ...
  mutations: {
    ...
    initialiseStore(store) {
      if (localStorage.getItem("newSettingsBackground")) {
        store.setting.background = localStorage.getItem(
          "newSettingsBackground"
        );
      }
      if (localStorage.getItem("newSettingsBorder")) {
        store.setting.border = localStorage.getItem("newSettingsBorder");
      }
      if (localStorage.getItem("newControlBackground")) {
        store.control.background = localStorage.getItem("newControlBackground");
      }
      if (localStorage.getItem("newControlBorder")) {
        store.control.border = localStorage.getItem("newControlBorder");
      }
    },

    //after save
    updateSettingsPanelsColour(state, pay) {
      state.setting = pay;
      localStorage.setItem("newSettingsBackground", state.setting.background);
      localStorage.setItem("newSettingsBorder", state.setting.border);
    },
    updateControlPanelsColour(state, pay) {
      state.control = pay;
      localStorage.setItem("newControlBackground", state.control.background);
      localStorage.setItem("newControlBorder", state.control.border);
    },
    ...
    setConnectingProgress: (state, progress) =>
      (state.connectingProgress = progress),
    ...
    setDisconnecting: (state, disconnecting) =>
  ...
  plugins: [connector.installStore, observer],
...
export function registerPluginData(plugin, dataType, key, defaultValue) {
  switch (dataType) {
    case PluginDataType.globalSetting:
      store.commit("settings/registerPluginData", {
        plugin,
        key,
        defaultValue,
      });
    ...
    case PluginDataType.machineCache:
      ...
      for (let machine in machines) {
        store.commit(`machines/${machine}/cache/registerPluginData`, {
          plugin,
          key,
          defaultValue,
        });
    ...
    case PluginDataType.machineSetting:
      ...
      for (let machine in machines) {
        store.commit(`machines/${machine}/settings/registerPluginData`, {
          plugin,
          key,
          defaultValue,
        });
