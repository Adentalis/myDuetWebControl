    ...
    <div :class="{ 'full-screen': fullscreen }" class="viewer-box">
      <div class="emergency-button-placement" v-show="fullscreen">
        <code-btn
          ...
          ><v-icon>mdi-flash</v-icon></code-btn
        >
      </div>
      <canvas
        ref="viewerCanvas"
      ...
      >
        ...
        <v-btn
          ...
          ><v-icon color="red">mdi-cancel</v-icon></v-btn
        >
      </div>
      <v-navigation-drawer
        v-model="drawer"
        ...
        <v-expansion-panels>
          <v-expansion-panel @click="scrollIntoView">
            <v-expansion-panel-header
              :title="$t('plugins.gcodeViewer.renderQuality.title')"
            >
              <v-icon class="mr-2">mdi-checkerboard</v-icon>
              <strong>{{
                $t("plugins.gcodeViewer.renderQuality.caption")
              }}</strong>
            </v-expansion-panel-header>
            <v-expansion-panel-content eager>
              ...
              >
                <v-btn block :value="1" :disabled="loading">{{
                  $t("plugins.gcodeViewer.sbc")
                }}</v-btn>
                <v-btn block :value="2" :disabled="loading">{{
                  $t("plugins.gcodeViewer.low")
                }}</v-btn>
                <v-btn block :value="3" :disabled="loading">{{
                  $t("plugins.gcodeViewer.medium")
                }}</v-btn>
                <v-btn block :value="4" :disabled="loading">{{
                  $t("plugins.gcodeViewer.high")
                }}</v-btn>
                <v-btn block :value="5" :disabled="loading">{{
                  $t("plugins.gcodeViewer.ultra")
                }}</v-btn>
                <v-btn block :value="6" :disabled="loading">{{
              ...
              ></v-checkbox>
              <v-checkbox
                v-model="liveTrackingShowSolid"
                :disabled="!isJobRunning || loading || !visualizingCurrentJob"
                :label="$t('plugins.gcodeViewer.showSolid')"
              ></v-checkbox>
              <v-checkbox
                v-model="spreadLines"
                :label="$t('plugins.gcodeViewer.spreadLines')"
              ></v-checkbox>
            </v-expansion-panel-content>
          </v-expansion-panel>
          <v-expansion-panel @click="scrollIntoView">
            <v-expansion-panel-header
              :title="$t('plugins.gcodeViewer.extruders.title')"
            >
            ...
            <v-expansion-panel-content>
              <v-btn
                ...
                color="primary"
                :title="$t('plugins.gcodeViewer.reloadView.title')"
                >{{ $t("plugins.gcodeViewer.reloadView.caption") }}</v-btn
              >
              <v-card v-for="(extruder, index) in extruderColors" :key="index">
                <v-card-title>
                  <h3>
                    {{ $t("plugins.gcodeViewer.tool", [index]) }}
                  </h3></v-card-title
                >
                <v-card-text>
              ...
              <v-card>
                <v-btn
                  ...
                  color="warning"
                  >{{
                    $tc("plugins.gcodeViewer.resetColor", extruderColors.length)
                  }}</v-btn
                >
              </v-card>
            </v-expansion-panel-content>
          ...
          <v-expansion-panel @click="scrollIntoView">
            <v-expansion-panel-header
              :title="$t('plugins.gcodeViewer.renderMode.title')"
            >
              <v-icon class="mr-2">mdi-palette</v-icon>
              <strong>{{
                $tc("plugins.gcodeViewer.renderMode.caption", 2)
              }}</strong>
            </v-expansion-panel-header>
            <v-expansion-panel-content>
              <v-card>
                ...
                <v-btn-toggle
                  ...
                  class="btn-toggle d-flex"
                >
                  <v-btn block :value="0" :disabled="loading">{{
          ...
          <v-expansion-panel @click="scrollIntoView">
            <v-expansion-panel-header
              :title="$t('plugins.gcodeViewer.progress.title')"
            >
            ...
            <v-expansion-panel-content>
              <v-card>
                ...
                ></v-slider>
                <v-checkbox
                  v-model="liveZTracking"
                  :label="$t('plugins.gcodeViewer.liveZTracking')"
                ></v-checkbox>
              </v-card>
              <v-card>
                <v-card-title>{{
                  $t("plugins.gcodeViewer.progressColor")
                }}</v-card-title>
                <v-card-text>
                  <gcodeviewer-color-picker
          ...
          <v-expansion-panel @click="scrollIntoView">
            ...
            <v-expansion-panel-content>
              <v-card>
                <v-card-title>{{
                  $t("plugins.gcodeViewer.background")
                }}</v-card-title>
                <v-card-text>
                  <gcodeviewer-color-picker
              ...
              <v-card>
                <v-card-title>{{
                  $t("plugins.gcodeViewer.bedRenderMode")
                }}</v-card-title>
                <v-card-text>
                  <v-btn-toggle
    ...
    <v-dialog v-model="objectDialogData.showDialog" max-width="300">
      <v-card>
        ...
        <v-card-actions>
          <v-row no-gutters>
            <v-col cols="6">
              <v-btn block @click="objectDialogCancelObject" color="primary"
                >Ok</v-btn
              >
            </v-col>
            <v-col cols="6">
...
export default {
  ...
  computed: {
    ...
    visualizingCurrentJob: function (state) {
      try {
        return (
          state.job.file.fileName === this.selectedFile && this.isJobRunning
        );
      } catch {
        return false;
      }
    ...
    canCancelObject() {
      if (
        ...
        !this.job ||
        !this.job.build ||
        this.job.build.objects.length == 0
      ) {
        return false;
      }
    ...
    jobSelectionLabel() {
      var selectionLabel = this.$t(
        "plugins.gcodeViewer.showObjectSelection.caption"
      );
  ...
  mounted() {
    ...
    if (viewer.lastLoadFailed()) {
      ...
      this.$makeNotification(
        "warning",
        "GCode Viewer",
        this.$t("plugins.gcodeViewer.renderFailed"),
        5000
      );
  ...
  watch: {
    ...
    sliderHeight: function (newValue) {
      if (this.sliderBottomHeight > newValue)
        this.sliderBottomHeight = newValue - 1;
